...
git commit -m "msg" (Sealing & Labeling):
...
Socho tum ek game khel rahe ho aur ek mushkil level aayi. Tum "Save Game" karte ho aur risk lete ho.
Coding mein hum Main code ko kharab nahi karte, hum ek Branch (duplicate duniya) banate hain.
...
Kaam karo, Add, Commit, Push:

Jab kaam khatam ho jaye, code ko GitHub pe bhej do (git push -u origin new-feature).
...
Laptop pe wapas "Main" branch pe aao: git switch main

Naya code download karo: git pull

Purani branch delete kar do (kaam ho gaya na): git branch -d new-feature
...
git stash (Dark mode ka code jeb mein rakho, folder clean ho jayega).

Bug fix karo, commit karo, push karo.

git stash pop (Jeb se wapas Dark mode ka code nikalo aur kaam continue karo).
